    ...
    async def async_step_user(self, user_input=None):
	    ...
	    return self.async_show_form(
	        ...
	        data_schema=vol.Schema({
	            ...
	            vol.Required("irrigation_zone_switch_entity_id"): selector({
	                "entity": {"domain": "switch"}
	            }),
	            vol.Required("irrigation_pump_switch_entity_id"): selector({
	                "entity": {"domain": "light"}
	            }),
	            vol.Required("water_consumption_entity_id"): selector({
	                "entity": {"domain": "sensor"}
	            }),
	            vol.Required("volume_required"): vol.Coerce(float),
	        }),
	        # Pass descriptions as `extra` UI hints
	        description_placeholders={
	            "name": "Name of the irrigation zone",
	            "irrigation_zone_switch_entity_id": "Select the switch entity for the irrigation zone",
	            "irrigation_pump_switch_entity_id": "Select the light entity which controls the 	irrigation pump",	
            "water_consumption_entity_id": "Select the sensor entity for water consumption",
          	  "volume_required": "Volume of water required to turn off the irrigation (in liters)",
        	}
    	)
